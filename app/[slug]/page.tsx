...
const getRelated = cache(async (currentSlug: string): Promise<{ popular: RelatedPost[]; recent: RelatedPost[] }> => {
  ...
  const all = [...db, ...statics].filter((p) => p.href !== `/${currentSlug}`)
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-10">
        ...
        <article className="lg:col-span-2 min-w-0">
          ...
          <div className="flex items-center justify-between gap-3 mb-6">
            ...
            <BookmarkButton article={{ slug: post.slug, title: post.title, excerpt: post.excerpt || post.meta_description || '', image: post.featured_image || '', category: post.category, href: `/${post.slug}` }} />
          </div>
